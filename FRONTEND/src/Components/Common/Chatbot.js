  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'assistant',
      text: `Hello ${user?.name || 'there'}! 👋 I'm FinBot, your personal financial assistant. How can I help you today?`,
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async (text = inputMessage) => {
    ...
    try {
      const conversationHistory = newMessages.slice(0, -1).map(msg => ({
        role: msg.role === 'assistant' ? 'FinBot' : 'User',
        message: msg.text,
      }));
    ...
    <>
      {!isOpen && (
        ...
        >
          ...
          <span className="hidden md:inline font-medium">Ask FinBot</span>
        </button>
      )}
        ...
        <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col z-50 border border-gray-200">
          ...
          <div className="p-4 border-t border-gray-200 bg-white rounded-b-2xl">
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              FinBot is an AI assistant and may make mistakes.
            </p>
          </div>
        </div>
      )}
